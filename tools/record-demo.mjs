...
/* The storyboard. `at` is a section id (or 'top'); `hold` is milliseconds the
   frame stays on screen. Beat 1 is the switcher, beat 2 is the proof. */
const board = [
  // beat 1 — eight themes, one structure, dark ground
  ...THEMES.map((t, i) => ({ theme: t, ground: 'dark', at: 'ramp', hold: i === 0 ? 1800 : 1250 })),
  // beat 1b — the same structure on the light ground
  ...['detect', 'health', 'vantage', 'zeno'].map(t => ({ theme: t, ground: 'light', at: 'ramp', hold: 1000 })),
  // beat 2 — the contrast budget, recomputed live from the tokens
  { theme: 'zeno', ground: 'light', at: 'budget', hold: 1900 },
  { theme: 'weft', ground: 'dark', at: 'budget', hold: 1900 },
  { theme: 'textify', ground: 'dark', at: 'budget', hold: 1900 },
  { theme: 'zeno', ground: 'dark', at: 'budget', hold: 2200 },
];
...
for (const [i, shot] of board.entries()) {
  ...
  await page.waitForTimeout(260); // let the token transitions settle
  const name = `frame-${String(i).padStart(3, '0')}.png`;
  await page.screenshot({ path: path.join(outDir, name) });
  frames.push({ file: name, hold: shot.hold, theme: shot.theme, ground: shot.ground, at: shot.at });
  process.stdout.write(`  ${name}  ${shot.theme} · ${shot.ground} · ${shot.at}\n`);
}
